...
/*	
	public void run(){
		...
		NetworkTable table = inst.getTable("piTable");
		NetworkTableEntry piTest = table.getEntry("test");
		inst.startClientTeam(1359);
		while(true){
			try{
				Thread.sleep(1000);
			...
			boolean test = piTest.setBoolean(true);
		}
	}
*/	
  ...
  public static class MyPipeline implements VisionPipeline {
	...
	@Override	public void process(Mat source0) {
		...
		Mat cvApplycolormapSrc = source0;
		int cvApplycolormapColormap = Imgproc.COLORMAP_HSV;
		cvApplycolormap(cvApplycolormapSrc, cvApplycolormapColormap, cvApplycolormapOutput);

		// Step Blur0:
		Mat blurInput = cvApplycolormapOutput;
		...
		double[] hslThresholdHue = {153.77697841726618, 180.0};
		...
		double[] filterContoursSolidity = {89.02877697841728, 100.0};
		double filterContoursMaxVertices = 120.0;
  ...
  //public static void main(String... args) {
    ...
    if (server) {
      ...
      ntinst.startClientTeam(team);
		...
		//NetworkTable table = ntinst.getTable("cv");
		//NetworkTableEntry piTest;
    ...
    if (cameras.size() >= 1) {
      VisionThread visionThread = new VisionThread(cameras.get(0),
              new MyPipeline(), pipeline -> {
				...
				kNetwork.setTable(kCalculation.getXValue(), kCalculation.getDistanceFromTarget(), kCalculation.getAngleFromTarget());
      });
